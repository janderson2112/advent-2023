    ...
    if 'J' in hand:
        for key in PRIORITY_DICT.keys():
            ...
            for key in cardDict.keys():
                if cardDict[key] == 5:
                    possibleStrengths.append(6 + encoding)
                elif cardDict[key] == 4:
                    possibleStrengths.append(5 + encoding)
                elif cardDict[key] == 3:
                    threeFlag = True
                elif cardDict[key] == 2:
                    if twoFlag:
                        possibleStrengths.append(2 + encoding)
                    else:
                        twoFlag = True
            if twoFlag and threeFlag:
                possibleStrengths.append(4 + encoding)
            elif threeFlag:
                possibleStrengths.append(3 + encoding)
            elif twoFlag:
                possibleStrengths.append(1 + encoding)
            else:
                possibleStrengths.append(0 + encoding)
        return max(possibleStrengths)
    else:
        cardDict = defaultdict(int)
        for c in hand:
            cardDict[c] += 1
